...
export function KpiTrackingView(){
  ...
  return <div style={{display:'grid',gap:16}}>
    <TracePageHeader kicker="KPI · TRACKING" title="KPI per organisasi, outlet, atau individu karyawan." description="Perhitungan perubahan/target gap pakai mesin yang sama dengan Analytics (calculateKPI) — bukan logika duplikat." />
    ...
    {subjectType==='employee'&&clientId&&<TraceCard><label>Karyawan<select value={employeeId} onChange={e=>setEmployeeId(e.target.value)} style={inputStyle}><option value="">Pilih karyawan</option>{employees.map(e=><option key={String(e.id)} value={String(e.id)}>{String(e.name)}</option>)}</select></label></TraceCard>}
    {msg&&<div className="trace-muted" style={{fontSize:12}}>{msg}</div>}
    {clientId&&<>
      <TraceCard><strong>Definisi Indikator</strong>
      ...
      {trendIndicator&&<TraceCard><strong>Tren — {String(indicators.find(i=>String(i.id)===trendIndicator)?.name)}</strong><div style={{marginTop:8,display:'grid',gap:4}}>{trend.length===0?<TraceEmptyState message="Belum ada histori."/>:trend.map((t,idx)=><div key={idx} style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,fontSize:12,padding:'4px 0',borderTop:'1px solid rgba(23,23,23,.06)'}}><span>{String(t.period)}</span><span>{String(t.current_value)}</span><span className="trace-muted">Target {t.target_value==null?'—':String(t.target_value)}</span></div>)}</div></TraceCard>}
      <TraceCard><strong>Input Nilai KPI — {period}</strong>
      ...
      <TraceCard><strong>Hasil KPI Periode {period}</strong>
        <div style={{marginTop:10,display:'grid',gap:8}}>{results.length===0?<TraceEmptyState message="Belum ada nilai untuk kombinasi klien/periode/subjek ini."/>:results.map(r=><div key={r.id} style={{padding:12,border:'1px solid rgba(23,23,23,.08)',borderRadius:11}}>
