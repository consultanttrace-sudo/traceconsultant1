...
export function StockOpnameView(){
  ...
  return <div style={{display:'grid',gap:16}}>
    <TracePageHeader kicker="INVENTORY · STOCK OPNAME (FIFO)" title="Kartu stok FIFO, sesi opname dual-count, dan posting varians ke jurnal." description="Setiap penerimaan jadi lot FIFO tersendiri. Sesi opname men-snapshot posisi stok, lalu membandingkan dengan hitung fisik — varians butuh alasan sebelum bisa di-approve, dan approval butuh akun sebelum diposting ke jurnal." />
    ...
    {clientId&&<>
      <TraceCard><strong>Posisi Stok (FIFO)</strong><div style={{marginTop:10,display:'grid',gap:5,fontSize:12}}>{position.length===0?<div className="trace-muted">Belum ada stok.</div>:position.map(p=><div key={String(p.item_id)} style={{display:'grid',gridTemplateColumns:'1.4fr 1fr 1fr 1fr',gap:8,padding:'5px 0',borderTop:'1px solid rgba(23,23,23,.06)'}}><span>{itemName(String(p.item_id))}</span><span>{String(p.qty_on_hand)}</span><span>{money(Number(p.avg_unit_cost))}/unit</span><span>{money(Number(p.value_on_hand))}</span></div>)}</div></TraceCard>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <TraceCard><strong>Terima Stok (lot baru)</strong><div style={{display:'grid',gap:8,marginTop:10}}><select value={receive.itemId} onChange={e=>setReceive({...receive,itemId:e.target.value})} style={inputStyle}><option value="">Pilih bahan</option>{items.map(i=><option key={String(i.id)} value={String(i.id)}>{String(i.item_name)}</option>)}</select><input type="number" placeholder="Qty" value={receive.qty} onChange={e=>setReceive({...receive,qty:e.target.value})} style={inputStyle}/><input type="number" placeholder="Unit cost" value={receive.cost} onChange={e=>setReceive({...receive,cost:e.target.value})} style={inputStyle}/><input placeholder="Referensi (PO/invoice)" value={receive.ref} onChange={e=>setReceive({...receive,ref:e.target.value})} style={inputStyle}/><button disabled={busy||!receive.itemId||!receive.qty} onClick={doReceive} style={{border:0,borderRadius:9,padding:'9px 12px',background:'#171717',color:'#fff',fontWeight:700}}>+ Terima</button></div></TraceCard>
        <TraceCard><strong>Keluarkan Stok</strong><div style={{display:'grid',gap:8,marginTop:10}}><select value={issue.itemId} onChange={e=>setIssue({...issue,itemId:e.target.value})} style={inputStyle}><option value="">Pilih bahan</option>{items.map(i=><option key={String(i.id)} value={String(i.id)}>{String(i.item_name)}</option>)}</select><input type="number" placeholder="Qty" value={issue.qty} onChange={e=>setIssue({...issue,qty:e.target.value})} style={inputStyle}/><input placeholder="Referensi (mis. konsumsi/waste)" value={issue.ref} onChange={e=>setIssue({...issue,ref:e.target.value})} style={inputStyle}/><button disabled={busy||!issue.itemId||!issue.qty} onClick={doIssue} style={{border:0,borderRadius:9,padding:'9px 12px',background:'#171717',color:'#fff',fontWeight:700}}>− Keluarkan</button></div></TraceCard>
      </div>
      <TraceCard><strong>Buka Sesi Opname Baru</strong><div className="trace-muted" style={{fontSize:12,marginTop:4}}>Snapshot posisi stok saat ini otomatis jadi "expected qty" untuk tiap bahan.</div><div style={{display:'grid',gridTemplateColumns:'1fr 2fr auto',gap:8,marginTop:10}}><input type="month" value={openForm.period} onChange={e=>setOpenForm({...openForm,period:e.target.value})} style={inputStyle}/><input placeholder="Catatan (opsional)" value={openForm.notes} onChange={e=>setOpenForm({...openForm,notes:e.target.value})} style={inputStyle}/><button disabled={busy} onClick={openSession} style={{border:0,borderRadius:9,padding:'9px 14px',background:'#171717',color:'#fff',fontWeight:700}}>Buka Sesi</button></div></TraceCard>
      <TraceCard><strong>Riwayat Sesi</strong><div style={{marginTop:10,display:'grid',gap:5}}>{sessions.length===0?<TraceEmptyState message="Belum ada sesi."/>:sessions.map(s=><div key={String(s.id)} onClick={()=>void loadLines(String(s.id))} style={{display:'grid',gridTemplateColumns:'1fr 1fr auto',gap:8,padding:'8px 0',borderTop:'1px solid rgba(23,23,23,.06)',fontSize:13,cursor:'pointer',background:sessionId===String(s.id)?'#fafaf8':'transparent'}}><span>{String(s.period_label)}</span><span className="trace-muted" style={{textTransform:'uppercase',fontSize:11}}>{String(s.status)}</span><ArrowRight size={14} className="trace-muted"/></div>)}</div></TraceCard>
      {selectedSession&&<TraceCard>
