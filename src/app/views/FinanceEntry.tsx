...
import { compareFinancePeriods, buildFinanceMonthlyView, assessFinanceQuality, filterFinanceRecordsByOutlet, type FinanceRecord, type FinanceStatementSection } from '../../core/finance';
...
import { getClientScope, setClientScope, useClientScope } from '../clientScope';
import { useScope } from '../scopeStore';
...
export function FinanceEntry(){
  ...
  const {outletId:scopeOutlet,period:scopePeriod}=useScope();
  const [draft,setDraft]=useState<FinanceDraft>({...EMPTY_FINANCE_DRAFT,clientId:getClientScope()});
  // A client chosen elsewhere (top bar, other module) pre-fills a NEW entry; editing an existing row keeps its own client.
  useEffect(()=>{setDraft(d=>d.id||d.clientId===scopeClient?d:{...d,clientId:scopeClient,outletId:scopeOutlet});},[scopeClient]); // eslint-disable-line react-hooks/exhaustive-deps
  // The global outlet/period pre-fill a NEW entry too (never an entry being edited, and never an outlet of another client).
  useEffect(()=>{setDraft(d=>d.id||d.outletId===scopeOutlet?d:{...d,outletId:scopeOutlet});},[scopeOutlet]);
  useEffect(()=>{setDraft(d=>d.id||!scopePeriod||d.period===scopePeriod?d:{...d,period:scopePeriod});},[scopePeriod]);
  ...
  return <div style={{display:'grid',gap:16}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'minmax(0,1.4fr) minmax(280px,.6fr)',gap:16}}>
      <TraceCard><div style={{display:'grid',gap:14}}>
        ...
        <label>Klien / Scope (wajib)<select value={draft.clientId} onChange={e=>{setDraft({...draft,clientId:e.target.value});setClientScope(e.target.value);}} style={inputStyle}><option value="">Pilih klien</option>{clients.map(c=><option key={String(c.id)} value={String(c.id)}>{String(c.name??c.business_name??c.id)}</option>)}</select></label>
      ...
      <TraceCard style={{alignSelf:'start'}}><div style={{display:'flex',gap:9,alignItems:'center'}}>{guidance.status==='ready'?<CircleCheck size={18}/>:<TriangleAlert size={18}/>}<strong>{guidance.title}</strong></div><p className="trace-muted" style={{fontSize:13,lineHeight:1.6}}>{guidance.message}</p>{issues.map(i=><div key={i.code} style={{padding:'10px 0',borderTop:'1px solid rgba(23,23,23,.08)'}}><strong style={{fontSize:12}}>{i.message}</strong><div className="trace-muted" style={{fontSize:12,marginTop:4}}>{i.suggestion}</div></div>)}{!draft.clientId.trim()&&<div className="trace-muted" style={{fontSize:12,marginTop:8}}>Isi Klien/Scope untuk mengaktifkan tombol Simpan.</div>}</TraceCard>
    </div>
    ...
    {draft.period&&comparison&&<TraceCard>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
