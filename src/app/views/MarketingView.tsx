...
export function MarketingView(){
  ...
  return <div style={{display:'grid',gap:16}}>
    <div className="trace-card" style={{padding:26}}><div className="trace-muted" style={{fontSize:12}}>MARKETING · CAMPAIGN</div><h1 style={{margin:'7px 0 5px',fontSize:30}}>Kampanye multi-channel dengan ROI, CAC, CTR otomatis.</h1><div className="trace-muted">Log tiap aktivitas (post, ad spend, promo, giveaway) per channel. ROI dan CAC dihitung dari data yang sama — bukan estimasi manual.</div></div>
    ...
    {clientId&&<>
      <div className="trace-card"><strong>Ringkasan per Channel ({rangeStart} → {rangeEnd})</strong>
        <div style={{marginTop:10,display:'grid',gap:5}}>{channelSummary.length===0?<div className="trace-muted" style={{fontSize:12}}>Belum ada data pada rentang ini.</div>:channelSummary.map((c,idx)=><div key={idx} style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr',gap:8,padding:'6px 0',borderTop:'1px solid rgba(23,23,23,.06)',fontSize:13}}>
      ...
      <div className="trace-card"><strong>Buat Kampanye</strong>
      ...
      <div className="trace-card"><strong>Daftar Kampanye</strong><div style={{marginTop:10,display:'grid',gap:5}}>{campaigns.length===0?<div className="trace-muted" style={{fontSize:12}}>Belum ada kampanye.</div>:campaigns.map(c=><div key={String(c.id)} onClick={()=>void loadCampaign(String(c.id))} style={{display:'grid',gridTemplateColumns:'1.4fr 1fr 1fr auto',gap:8,padding:'8px 0',borderTop:'1px solid rgba(23,23,23,.06)',fontSize:13,cursor:'pointer',background:campaignId===String(c.id)?'#fafaf8':'transparent'}}><span>{String(c.name)}</span><span className="trace-muted">{String(c.primary_channel??'—')}</span><span className="trace-muted" style={{textTransform:'uppercase',fontSize:11}}>{String(c.status)}</span><ArrowRight size={14} className="trace-muted"/></div>)}</div></div>

      {selectedCampaign&&<div className="trace-card">
