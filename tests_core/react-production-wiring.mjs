...
const checks=[
  ...
  ['Import approval is server-side state gated',intake.includes('Invalid import status transition') && intake.includes("current==='reviewed'")],
  ...
  ['Acquisition discovery uses durable jobs/checkpoints',main.includes('trace_jobs') && main.includes('trace_job_checkpoints') && main.includes('saveDiscoveryCheckpoint')],
  ...
  ['Accounting view reads accounts/journal via allowlisted resources',main.includes("['accounts','journal_entries','journal_lines']")],
