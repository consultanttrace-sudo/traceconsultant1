...
create policy trace_fnb_target_select on public.trace_fnb_target_plans for select to authenticated using (public.trace_is_team_member());
drop policy if exists trace_fnb_target_insert on public.trace_fnb_target_plans;
create policy trace_fnb_target_insert on public.trace_fnb_target_plans for insert to authenticated with check (public.trace_is_team_member());
drop policy if exists trace_fnb_target_update on public.trace_fnb_target_plans;
create policy trace_fnb_target_update on public.trace_fnb_target_plans for update to authenticated using (public.trace_is_team_member()) with check (public.trace_is_team_member());
drop policy if exists trace_fnb_target_delete on public.trace_fnb_target_plans;
create policy trace_fnb_target_delete on public.trace_fnb_target_plans for delete to authenticated using (public.trace_is_team_member());
...
begin
 if not public.trace_is_team_member() then raise exception 'client scope denied'; end if;
...
create or replace function public.trace_delete_fnb_target_plan(p_id uuid,p_client_id uuid) returns boolean language plpgsql security definer set search_path=public as $$ begin if not public.trace_is_team_member() then raise exception 'client scope denied'; end if; delete from public.trace_fnb_target_plans where id=p_id and client_id=p_client_id; return found; end $$;
