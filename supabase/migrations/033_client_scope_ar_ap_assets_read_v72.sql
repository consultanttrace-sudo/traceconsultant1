-- TRACE v72.3 — extend the single client read boundary with AR/AP/fixed
-- assets/period lock data, so main.tsx can render aging reports, depreciation
-- schedules, and lock status without any new direct-table access path.
...
begin
  ...
  return jsonb_build_object(
    ...
    'ar_invoices', coalesce((select jsonb_agg(to_jsonb(t) order by t.due_date) from public.trace_ar_invoices t where t.client_id=cid),'[]'::jsonb),
    'ar_payments', coalesce((select jsonb_agg(to_jsonb(t) order by t.paid_date desc) from public.trace_ar_payments t where t.client_id=cid),'[]'::jsonb),
    'ap_bills', coalesce((select jsonb_agg(to_jsonb(t) order by t.due_date) from public.trace_ap_bills t where t.client_id=cid),'[]'::jsonb),
    'ap_payments', coalesce((select jsonb_agg(to_jsonb(t) order by t.paid_date desc) from public.trace_ap_payments t where t.client_id=cid),'[]'::jsonb),
    'fixed_assets', coalesce((select jsonb_agg(to_jsonb(t) order by t.acquisition_date) from public.trace_fixed_assets t where t.client_id=cid),'[]'::jsonb),
    'period_locks', coalesce((select jsonb_agg(to_jsonb(t) order by t.period desc) from public.trace_period_locks t where t.client_id=cid),'[]'::jsonb)
  );
end; $$;
