...
async function run() {
  ...
  //    localStorage, returns false (truthful contract: a local recovery
  //    copy is NOT an authoritative cloud commit), and forces an alert()
  //    the user must click through -- not a dismissable/missable toast.
  //    (index.html deliberately switched this from toast() to alert() --
  //    see the comment above the alert() call in saveDataUnlocked's catch
  //    block -- specifically so this can't be missed while a long batch
  //    process like Discovery is running unattended.)
  // ---------------------------------------------------------------
  {
    ...
    const alertMsg = alerts[alerts.length - 1] || "";
    assert(/belum dianggap tersimpan/i.test(alertMsg),
      "6c) user must be forced to acknowledge sync failure via alert() (not a silent/missable success toast), got: " + JSON.stringify(alertMsg));
  }
